  ...
  "databases": [
    {
      ...
      "v8path": "C:\\Program Files\\1cv8\\8.3.27.1688\\bin", // платформа именно этой базы (см. ниже)
...
| `v8path` | string | нет | Платформа этой базы — перекрывает корневой `v8path` (см. ниже) | Руками |
...
### Платформа базы (`v8path`)

Корневой `v8path` — платформа проекта, `databases[].v8path` — платформа **этой** базы. Порядок
выбора у всех навыков, запускающих платформу:

1. явный `-V8Path` в вызове;
2. `v8path` записи базы, соответствующей переданному соединению;
3. корневой `v8path`;
4. автопоиск — **самая старшая** установленная платформа.

Запись ищет сам скрипт: сопоставляет параметры соединения с `databases[]` — файловую базу по
`path`, серверную по паре `server` + `ref`. Передавать `-V8Path` вручную не нужно.

Поле закрывает два случая:

- **базы проекта живут на разных версиях платформы** — каждая открывается своей;
- **формат XML-выгрузки** (`version` в `Configuration.xml`) задаёт та платформа, **которая
  выгружает**; режим совместимости конфигурации на него не влияет. Выгрузка с 8.5 даёт формат 2.21,
  и коллега на 8.3.27 её уже не загрузит: «Неизвестная версия формата». Прибив базе платформу, вы
  прибиваете и формат её исходников. Без поля сработает автопоиск и возьмёт самую свежую
  установленную платформу — то есть самый новый формат.

Значение — тот же вид пути, что и у корневого: каталог `bin`, либо сам `1cv8.exe`/`ibcmd.exe`
(см. «Движок: 1cv8 или ibcmd»). Путь машинный, а `.v8-project.json` у каждого свой — договориться
внутри команды о версии всё равно придётся, поле лишь не даёт ошибиться при переключении между
базами.
...
Платформу для найденной базы подставляет сам скрипт — см. «Платформа базы (`v8path`)».
...
- **в файле настроек** — прописать в `v8path` не каталог `bin`, а сам файл `...\\bin\\ibcmd.exe` (тогда через
  `ibcmd` пойдут все операции). То же работает и в `v8path` записи базы — тогда через `ibcmd`
  пойдут операции только с ней.
