...
Внутри `DefinedType` тип-множество **запрещено**: платформа отвечает «Недопустимый тип» и на
`AnyRef`, и на голый `CatalogRef`, и на `Characteristic.Xxx`, и на вложенный `DefinedType.Xxx`
(замерено на 8.3.24.1691 загрузкой в базу; контрольный вариант с обычным `CatalogRef.Xxx` принят).
Состав определяемого типа — только конкретные типы.
