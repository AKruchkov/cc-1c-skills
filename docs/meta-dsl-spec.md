...
**Где множество принимают.** Три уровня, все замерены на 8.3.24.1691 загрузкой в базу:

| Контейнер | Что можно | Чем отвечает платформа |
|-----------|-----------|------------------------|
| `DefinedType` (состав) | только конкретные типы | «Недопустимый тип» и файл не грузится — на `AnyRef`, голый `CatalogRef`, `Characteristic.Xxx` и вложенный `DefinedType.Xxx`; контрольный `CatalogRef.Xxx` принят |
| ПВХ (`Type`/тип значения) | из множеств — только `DefinedType.Xxx` | остальное тоже грузится, но Конфигуратор не предлагает: голого метатипа и `AnyRef` в дереве выбора нет вовсе, `Characteristic.Xxx` — тоже (тип значения характеристики не может быть значением характеристики). `AnyRef` возвращается с выгрузки как `AnyIBRef`. В корпусе erp+acc ни один из 24 ПВХ множеств в типе значения не использует |
| любой составной тип | `DefinedType.Xxx` и `Characteristic.Xxx` — только ЕДИНСТВЕННЫМ | одним из составного Конфигуратор выбрать не даёт (характеристику — даже вместе с другой характеристикой), но загрузчик принимает: в корпусе erp+acc 6500 определяемых единственным против 1 составного (и этот один — в типовой ERP, `Документ.НачислениеИСписаниеБонусныхБаллов.Баллы`), 1741 характеристика единственным против 0 составных. Голые метатипы это НЕ затрагивает: `DocumentRef + CatalogRef` — обычный составной тип, 525 вхождений в поставке |

Отсюда разная строгость проверок: состав определяемого типа — ошибка, остальные два — предупреждение.
