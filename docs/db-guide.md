...
- **в файле настроек** — прописать в `v8path` не каталог `bin`, а сам файл `...\bin\ibcmd.exe`
  (тогда через `ibcmd` пойдут все операции). Это работает и в `v8path` отдельной базы — там же
  задаётся её платформа, если базы проекта живут на разных версиях
  (см. [конфигурацию проекта](v8-project-guide.md#платформа-базы-v8path)).
